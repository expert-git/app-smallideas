 ...
 */
	...
	include_once(EPABSPATH."/include/classes/BO/userFavourite.class.php");
	include_once(EPABSPATH."/include/classes/BO/voucher.class.php");
	
	class DAL_UserFavourite {
		...
		static public function GetAllForUser($id) {
			...
			{		
				...
				$sqlStatement = 'SELECT v.* FROM `userFavourite` u LEFT JOIN `voucher` v ON u.voucherId = v.id WHERE userId = :id 
		...
		static public function GetByUserAndVoucher($userId,$voucherId) {
			...
			{		
				$oDbConnection = new DAL_PDO();			
				$sqlStatement = 'SELECT * FROM `userFavourite` 
								WHERE userId = :userId
				...
				if($sth->rowCount()>0) return new BO_UserFavourite($rsResults);
			}
			catch (Exception $ex)
			{			
				throw $ex;	
		...
		static public function Create($userId,$voucherId) {
			...
			{	
				...
				$oDbConnection = new DAL_PDO();			
				$sqlStatement = 'INSERT INTO `userFavourite` (userId,voucherId) VALUES (:userId,:voucherId)';
				$sth = $oDbConnection->prepare($sqlStatement, array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));			
				$sth->execute(array(										
					':userId' => $userId,
					':voucherId' => $voucherId				
		...
		static public function Delete($userId,$voucherId) {
			...
			{	
				...
				$oDbConnection = new DAL_PDO();			
				$sqlStatement = 'DELETE	FROM `userFavourite`							
