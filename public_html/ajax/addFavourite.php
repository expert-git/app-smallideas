...
//			echo "$userId - $vourcherId";
			$userFavouriteObj = DAL_UserFavourite::GetByUserAndVoucher($userId,$voucherId);

			if(!$userFavouriteObj){
				//doesnt exist, so add favourite
				$result = DAL_UserFavourite::Create($userId,$voucherId);
				if($result){
				...
				//exists, so delete
				$result = DAL_UserFavourite::Delete($userId,$voucherId);
				if($result){
